...
export default function Home() {
  const [timers,setTimers]=useState<Timer[]>([]),[now,setNow]=useState(0),[server,setServer]=useState<Server>(SERVERS[0]);
  ...
  const add=(e:FormEvent)=>{e.preventDefault();const at=new Date(appeared).getTime();if(!monster.trim())return setError("請輸入魔物名稱");if(!Number.isFinite(at))return setError("請輸入正確的日期與時間");if(at>Date.now()+60000)return setError("出現時間不可晚於現在");setTimers(x=>[...x,{id:crypto.randomUUID(),server,monster:monster.trim(),multiplier,appearedAt:at,createdAt:Date.now()}]);setMonster("");setAppeared(localValue());setError("");};
  ...
  return <main className="site-shell"><InkCanvas tone={tone} auto={auto} wash={wash}/><div className="paper-grain"/>
    <div className="content-wrap">
      <header><div className="brand"><span className="seal">水藍</span><div><p className="eyebrow">CROSSGATE ・ RESPAWN &amp; EXP RATE</p><h1>魔物重生・經驗倍率帖</h1></div></div><button className="ghost" onClick={()=>setAdminOpen(x=>!x)}>{admin?"管理中":"管理員登入"}</button></header>
      <section className="hero"><div className="hero-copy"><p className="vertical">水藍魔力寶貝</p><div><h2>記下現身一刻，<br/>倍率高者先行。</h2><p>六大伺服器共用・跨日安全計時・依經驗倍率與剩餘時間自動排序。</p></div></div>
        <form className="timer-form" onSubmit={add}><div className="form-head"><b>新增狩獵紀錄</b><button type="button" onClick={()=>setAppeared(localValue())}>套用現在時間</button></div><div className="fields">
          ...
          <label><span>出現時間（24 小時制）</span><input type="datetime-local" value={appeared} max={localValue()} onChange={e=>setAppeared(e.target.value)}/></label><button className="primary">開始計時 →</button></div>{error&&<p className="error">{error}</p>}</form>
      </section>
      ...
      <section className="queue"><div className="section-head"><div><p className="kicker">RESPAWN &amp; EXP QUEUE</p><h3>重生與倍率候時錄</h3></div><div className="summary"><b>{active}</b> 計時中　・　<b>{timers.length}</b> 全部紀錄</div></div>
        ...
        <div className="timer-list">{ordered.length===0?<div className="empty"><span>◯</span><h4>候時錄尚無墨跡</h4><p>在上方輸入魔物出現時間與倍率，即會自動計算三小時重生並排序。</p></div>:ordered.map((x,i)=>{const end=x.appearedAt+THREE_HOURS,left=Math.max(0,end-now),expired=left<=0;return <article className={`timer-card ${expired?"expired":""}`} key={x.id}><div className="rank">{String(i+1).padStart(2,"0")}</div><div className="monster"><div className="monster-meta"><span>{x.server}</span><b>{x.multiplier.toFixed(1)} 倍經驗</b></div><h4>{x.monster}</h4><p>現身 {dateTime(x.appearedAt)} ・ 重生 {dateTime(end)}</p></div><div className="count"><span>{expired?"已可重生":"距離重生"}</span><strong>{clock(left)}</strong><div><i style={{width:`${Math.min(100,left/THREE_HOURS*100)}%`}}/></div></div><button className="remove" onClick={()=>setTimers(t=>t.filter(y=>y.id!==x.id))} aria-label={`刪除 ${x.monster}`}>×</button></article>;})}</div>
