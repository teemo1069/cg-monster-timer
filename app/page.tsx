...
export default function Home() {
  ...
  return <main className="site-shell"><InkCanvas tone={tone} auto={auto} wash={wash}/><div className="paper-grain"/>
    <div className="content-wrap">
      <header className="masthead"><div className="brand"><span className="brand-seal">時</span><div><b>水藍魔力寶貝</b><small>MONSTER RESPAWN ARCHIVE</small></div></div><button className="ghost" onClick={()=>setAdminOpen(x=>!x)}>{admin?"管理中":"管理員登入"}</button></header>
      <section className="hero">
        ...
        <div className="hero-copy"><p className="poster-index"><span>水藍六服</span><b>THREE HOUR CYCLE</b><span>候時錄</span></p><h1>魔物重生<em>計時器</em></h1><div className="poster-credit"><span>出現時刻</span><i/><span>經驗倍率</span><i/><span>自動排序</span></div></div>
        <form className="timer-form" onSubmit={add}><div className="form-head"><div><span>NEW RECORD</span><b>新增狩獵紀錄</b></div><button type="button" onClick={()=>setAppeared(localValue())}>套用現在時間</button></div><div className="fields">
