  ...
  return <main className="site-shell"><InkCanvas tone={tone} auto={auto} wash={wash}/><div className="paper-grain"/>
    <div className="content-wrap">
      ...
      <section className="hero">
        <div className="poster-scene" aria-hidden="true"><i className="poster-mist"/><i className="poster-brush"/></div>
        <div className="hero-copy"><h1>魔物重生計時器</h1></div>
      ...
      <section className="queue"><div className="section-head"><h3>重生候時錄</h3><div className="summary"><span><b>{active}</b> 計時中</span><i/><span><b>{timers.length}</b> 全部</span></div></div>
        <div className="servers">{SERVERS.map((x,i)=><span key={x}><b>{String(i+1).padStart(2,"0")}</b>{x}</span>)}</div>
